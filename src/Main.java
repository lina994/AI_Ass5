...
public class Main {
	...
	public static void main(String[] args) {
		...
		while (true) {
			try {
				...
				startVertex--;
				if(startVertex>=0 && startVertex<=numOfVertices-1)break;
		...
		while (true) {
			try {
				...
				if(utilityTypeCalc==1 || utilityTypeCalc==2)break;
	...
	private static void createStateArr(BeliefState st_parent, int st_nextVertex, int[] st_people, int st_saved, int st_peopleInCar,
			Edge[] st_uncertainEdges, int st_time) {
		...
		if(k==0) { 
			...
			statesArr.add(st);
			st.prob[0]=1;
			st.arr[0]=new BeliefState(st_nextVertex, st_people.clone(), st_saved, st_peopleInCar, Edge.deepEdgeArrCopy(st_uncertainEdges), 
					st_time, 0);
			...
			for (int i = 0; i < arrTF.length; i++) { //move on rows
				...
				for (int j = 0; j < st_uncertainEdges.length; j++) {
					if(st_uncertainEdges[j].v1==st_nextVertex || st_uncertainEdges[j].v2==st_nextVertex) {
						if(st_uncertainEdges[j].blockageProbability!=0 && st_uncertainEdges[j].blockageProbability!=1) {
				...
				st.arr[i]= new BeliefState(st_nextVertex, st_people.clone(), st_saved, st_peopleInCar, un_ed, st_time, 0);
 ...
 public static BeliefState getStateFromList(BeliefState s1) {
    ...
    private static void createStateArrUsingMinMax(BeliefState st_parent, int st_nextVertex, int[] st_people, int st_saved, int st_peopleInCar,
			Edge[] st_uncertainEdges, int st_time) {
		...
		if(k==0) { 
			...
			st.prob[0]=1;
			st.arr[0]=new BeliefState(st_nextVertex, st_people.clone(), st_saved, st_peopleInCar, Edge.deepEdgeArrCopy(st_uncertainEdges), 
					st_time, Double.NEGATIVE_INFINITY);
		...
		// num of belief state in state arr = 2^|uncertainEdges|
			...
			for (int i = 0; i < arrTF.length; i++) { //move on rows
				...
				st.arr[i]= new BeliefState(st_nextVertex, st_people.clone(), st_saved, st_peopleInCar, un_ed, st_time, Double.NEGATIVE_INFINITY);
				BeliefState prev=getStateFromList(st.arr[i]);
				if(prev==null)
					states.add(st.arr[i]);
				else {
